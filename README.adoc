...
$docker run -p 8083:8083 --volumes-from keys --env JENKINS_URL="MY_JENKINS_URL" --env GITHUB_TOKEN="MY_GITHUB_TOKEN" --env SLACK_TOKEN="MY_SLACK_TOKEN" --name jks myjenkins
----

Where:

 - __MY_JENKINS_URL__: is the external URL of your jenkins (ex. "https://ci-james.org:8083/")
...
----
$firefox https://`docker inspect --format {{.NetworkSettings.IPAddress}} jks`:8083
or
